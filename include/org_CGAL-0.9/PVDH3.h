...
template < class FT, class RT >
ostream &operator<<(ostream &os, const CGAL_VectorH3<FT,RT> &p)
{
    switch(os.iword(CGAL_IO::mode)) {
    case CGAL_IO::ASCII :
        return os << p.hx() << ' ' << p.hy() << ' ' << p.hz() << ' ' << p.hw();
...
template < class FT, class RT >
istream &operator>>(istream &is, CGAL_VectorH3<FT,RT> &p)
{
    RT hx, hy, hz, hw;
    switch(is.iword(CGAL_IO::mode)) {
    case CGAL_IO::ASCII :
    ...
    p = CGAL_VectorH3<FT,RT>(hx, hy, hz, hw);
    return is;
}
