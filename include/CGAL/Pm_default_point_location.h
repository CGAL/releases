...
class CGAL__X_curve_plus: public Planar_map::Traits::X_curve
{
public:
  ...
  CGAL__X_curve_plus() : curve(),parent() {};
  CGAL__X_curve_plus(const Point& p,const Point& q) : curve(p,q),parent() {};
  CGAL__X_curve_plus(const curve &cv,const Halfedge_handle& p) : curve(cv),parent(p){}
  CGAL__X_curve_plus(const Halfedge_handle& p) : curve(p->curve()),parent(p){}
  // used when no Halfedge_handle is supplied.
  CGAL__X_curve_plus(const curve &cv) : curve(cv),parent() {};
  CGAL__X_curve_plus(const CGAL__X_curve_plus &cv) : curve(cv),parent(cv.parent){}
  ~CGAL__X_curve_plus(){}
  CGAL__X_curve_plus& operator=(const CGAL__X_curve_plus &cv)
  {
    curve::operator=(cv);
...
template <class Planar_map>
class CGAL_Pm_default_point_location : public CGAL_Pm_point_location_base<Planar_map> {

protected:
 
  typedef CGAL__X_curve_plus<Planar_map> X_curve_plus;


  typedef typename Planar_map::Traits_wrap Traits_wrap;
  typedef CGAL_Trapezoidal_map_2<Traits_wrap,X_curve_plus> TM;
  typedef Planar_map PM;


public:
	CGAL_Pm_default_point_location(bool preprocess=false) 
  ...
  ~CGAL_Pm_default_point_location() {}
  /* 
	Postcondition:
  ...
  */
  ...
  {
    //trying
    //there are different internal compiler errors between the two
...
*/
	bool halfedge_represents_point_inside_face(const Halfedge_handle& h,const Point& p) const 
	{
