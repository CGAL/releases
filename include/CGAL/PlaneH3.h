...
class CGAL_PlaneH3 : public CGAL_Handle
{
 public:
    ...
    bool                operator==( const CGAL_PlaneH3& )    const;
    bool                operator!=( const CGAL_PlaneH3& )    const;
    bool                identical ( const CGAL_PlaneH3& )    const;
    int                 id() const;
...
template < class FT, class RT >
CGAL_PlaneH3<FT,RT>::CGAL_PlaneH3(const CGAL_PlaneH3 & tbc)
 : CGAL_Handle(tbc)
{
}
...
CGAL_PlaneH3<FT,RT>::transform_to_2d() const
{
 ...
 assert((   ortho.transform(rotate_and_more)
        == CGAL_PointH3<FT,RT>( RT(0.0), RT(0.0), RT(1.0)) ));
 assert((   e1p.transform(rotate_and_more)
        == CGAL_PointH3<FT,RT>( RT(1.0), RT(0.0), RT(0.0)) ));
 assert((   e2p.transform(rotate_and_more)
