...
#include <CGAL/Cordertype.h>

template < class FT >
class CGAL_Line_2pC2 : public handle_base
{
public:
                       CGAL_Line_2pC2();
                       ...
                       ~CGAL_Line_2pC2();
  //                   operator CGAL_Line_2pC2<double>() const;
...
template < class FT >
inline CGAL_Line_2pC2<FT>::CGAL_Line_2pC2(const FT &a, const FT &b, const FT &c)
{ 
  CGAL_PointC2<FT> p;
  if (b==FT(0.0)) 
    {
      p = CGAL_PointC2<FT>( (b-c)/a, FT(1.0) );
    }
  else
    {
      p = CGAL_PointC2<FT>( FT(1.0), -(a+c)/b );
    } 
...
template < class FT >
CGAL_Line_2pC2<FT>::operator CGAL_Line_2pC2<double>() const
{
  return CGAL_Line_2pC2<double>(first_point(), second_point());
}
