...
//
// file          : include/CGAL/Trapezoidal_map_2.h
// package       : pm (2.052)
// source        : 
// revision      : 
// revision_date : 
...
#define CGAL_TRAPEZOIDAL_MAP_2_H
...
#include <CGAL/Trapezoidal_map_2_misc.h>
#endif

/* syntax:				
CGAL_Trapezoidal_map_2<Traits,X_curve_plus> 			*/
template < class Traits,class X_curve_plus> class CGAL_Trapezoidal_map_2;
...
trapezoidal decomposition and vertical decomposition.
 ...
 For requirements on Traits and X_curve_plus classes see Trapezoidal_map_2 documentation.
*/
template < class Traits,class X_curve_plus>
class CGAL_Trapezoidal_map_2
{
public:
	...
	struct Unbounded {};
	typedef CGAL_Trapezoidal_map_2<Traits,X_curve_plus> Self;
	...
	typedef map<int,pointer> pointer_container;
	typedef CGAL_DAG< X_trapezoid, Active_right_degenerate_curve_trapezoid> Data_structure;
	...
	*/
...
#ifndef CGAL_TRAPEZOIDAL_MAP_2_NO_WARNING
	...
	/* syntax:				
	CGAL_Trapezoidal_map_2<Traits,X_curve_plus>::In_face_iterator */
	...
	public:
		...
		In_face_iterator(Traits* traits_,const curve& sepc,pointer currt=0) :
		  ...
		  {
			  return Base_trapezoid_iterator::operator==(it)&& traits->curve_is_same(sep,it.sep);
		  }
...
#ifndef CGAL_TRAPEZOIDAL_MAP_2_DEBUG
#ifndef CGAL_TRAPEZOIDAL_MAP_2_NO_WARNING
...
#ifdef CGAL_TRAPEZOIDAL_MAP_2_DEBUG
	...
	//////////////////////////////////////////////
        //CGAL_Trapezoidal_map_2 member functions:
	//////////////////////////////////////////////
...
output: trapezoid iterator
 ...
 Description:
 the output (trapezoid iterator) is initialized with the leftmost and rightmost
 (non degenerate) trapezoids in the trapezoid interval that corresponds to the input from either the top side or the bottom side, depending on the up flag.
 preconditions:
 There exist non degenerate trapezoids between the roots of the input DS's
	*/
	In_face_iterator follow_curve(
		...
		const curve& cv,
		CGAL_Comparison_result up) const
	{
		...
		{
			cerr << "\ncv " << cv;
			cerr << "\nleft_end_point " << left_end_point << endl;
	...
	*/
		...
		)
	{
		...
		Data_structure leftDS= Data_structure(X_trapezoid(traits,curr.left(),p,curr.bottom(),curr.top(),
			curr.boundedness()&(CGAL_TRAPEZOIDAL_MAP_2_LEFT_UNBOUNDED|CGAL_TRAPEZOIDAL_MAP_2_BOTTOM_UNBOUNDED|CGAL_TRAPEZOIDAL_MAP_2_TOP_UNBOUNDED)
			));
		Data_structure rightDS=Data_structure(X_trapezoid(traits,p,curr.right(),curr.bottom(),curr.top(),
			curr.boundedness()&(CGAL_TRAPEZOIDAL_MAP_2_RIGHT_UNBOUNDED|CGAL_TRAPEZOIDAL_MAP_2_BOTTOM_UNBOUNDED|CGAL_TRAPEZOIDAL_MAP_2_TOP_UNBOUNDED)
			));
	...
	/* Description:
	the opposite operation for spliting the trapezoid with vertical line through p */
	/* Precondition:
		...
		const curve& cv)
	{
		...
		*/
		Data_structure
			...
			currt.top(),currt.boundedness()&(CGAL_TRAPEZOIDAL_MAP_2_LEFT_UNBOUNDED|CGAL_TRAPEZOIDAL_MAP_2_RIGHT_UNBOUNDED|CGAL_TRAPEZOIDAL_MAP_2_TOP_UNBOUNDED))),
			bottomBT=Data_structure(X_trapezoid(traits,currt.left(),currt.right(),
			currt.bottom(),cv,currt.boundedness()&(CGAL_TRAPEZOIDAL_MAP_2_LEFT_UNBOUNDED|CGAL_TRAPEZOIDAL_MAP_2_RIGHT_UNBOUNDED|CGAL_TRAPEZOIDAL_MAP_2_BOTTOM_UNBOUNDED)));
	...
	*/
	void replace_curve_at_point_using_geometry(reference t,reference sep,bool cv_top_right=true)
	{
		Point p(sep.left());
		curve cv(t.top());
		...
		{
			if (cv_top_right)
				while(traits->curve_compare_at_x_from_top(circ->top(),cv,p)!=CGAL_EQUAL) circ++;
				else
					while(traits->curve_compare_at_x_from_bottom(circ->bottom(),cv,p)!=CGAL_EQUAL) circ++;
					circ.replace(t);
		}
	}
	void insert_curve_at_point_using_geometry(reference sep,reference end_point)
	{
		...
		*/
		...
		if(!traits->point_is_same(end_point.left(),sep.right()))
		{
			...
			{
				/* set circ[0] to first curve on a counter clockwise sweep starting at cv */
				...
				while(traits->curve_compare_at_x_from_top(circ->top(),cv,p)==CGAL_SMALLER)
				{
					circ++;
					if (circ==stopper)
						break;
				...
				CGAL_assertion(traits->curve_compare_at_x_from_top(circ->top(),cv,p)!=CGAL_EQUAL);
				...
				{
					if(traits->curve_compare_at_x_from_bottom(rt->top(),cv,p)==CGAL_SMALLER)
						end_point.set_rt(&sep);
				}
			}
		}
		else
		{
			...
			else
			{
				...
				// otherwise advance as required
				while(traits->curve_compare_at_x_from_bottom(circ->top(),cv,p)==CGAL_SMALLER)
				{
					circ++;
					if (circ==stopper)
						break;
				...
				CGAL_assertion(traits->curve_compare_at_x_from_bottom(circ->top(),cv,p)!=CGAL_EQUAL);
				...
				{
					// set end_point.right_top_neighbour();
					if(traits->curve_compare_at_x_from_top(lb->top(),cv,p)==CGAL_SMALLER)
						end_point.set_lb(&sep);
				}
			}
		}
	...
	*/
	reference insert_curve_at_point_using_geometry(const curve& cv,const Point& p,pointer& tr,const Locate_type& lt)
	{
		...
		if (traits->curve_compare_at_x_from_top(cv,tr->top(),p)==CGAL_SMALLER)
			tr->set_top(cv);
		if (traits->curve_compare_at_x_from_bottom(cv,tr->bottom(),p)==CGAL_SMALLER)
			tr->set_bottom(cv);
		return *tr;
	}
	...
	void replace_curve_at_point_using_geometry(
		const curve& old_cv,const curve& new_cv,reference sep)
	{
	...
	/*
				...
				*/
	static Locate_type search_using_data_structure(Data_structure& curr,Traits* traits,const Point& p,const curve* cv,CGAL_Comparison_result up=CGAL_EQUAL)
	{
		const Point* pp;
		const curve* pc;
				...
				{  // p on CURVE
					...
					else
					{
						...
						case CGAL_EQUAL:
							...
							default:
								CGAL_assertion(up==CGAL_LARGER||up==CGAL_SMALLER||up==CGAL_EQUAL);
								return Locate_type();
							...
							default:
								CGAL_assertion(res==CGAL_LARGER||res==CGAL_SMALLER||res==CGAL_EQUAL);
								return Locate_type();
	...
	/*==============================================
	CGAL_Trapezoidal_map_2 public member functions
	==============================================*/
public:
	
  CGAL_Trapezoidal_map_2(bool preprocess=false) {init();set_needs_update(preprocess);}

	virtual ~CGAL_Trapezoidal_map_2()
	{
	...
	*/
	virtual X_trapezoid insert(const X_curve_plus& cv)
	{
		Point tmp;
...
// give initiale values to  bounding points when empty
	...
	*/
	...
	/*
	  ...
	  cout << "\nTM::insert_in_face_interior() begin" << endl;
	  cout << is_valid(*DS);
	  cout << *DS << endl;
		...
		*/
...
#ifndef CGAL_NO_TRAPEZOIDAL_MAP_2_OPTIMIZATION
		...
		In_face_iterator it=follow_curve(tt_p1,cv,CGAL_LARGER);
	...
	virtual void remove(const X_curve_plus& cv)
	{
	...
	/*
	  ...
	  cout << "\nTM::remove_in_face_interior() begin" << endl;
	  cout << is_valid(*DS);
	  cout << *DS << endl;
		...
		*/
		...
		Data_structure
			...
			&tt2=*t2.get_node();
		In_face_iterator
			bottom_it(follow_curve(tt1,cv,CGAL_SMALLER)),
			mid_it(follow_curve(tt1,cv,CGAL_EQUAL)),
			top_it(follow_curve(tt1,cv,CGAL_LARGER));
	...
	*/
	...
	curve
	precondition:
	c						The first input curve is the union of the other two.
	The intersection of the latter is a point inside the interior of the former.
	The latter are ordered from left-down to right-up
	postcondition:
	The first input curve is broken into two curves corresponding to the input.
	The output is the degenerate point trapezoid that corresponds to the
	splitting point.*/
	  ...
	  cout << *DS << endl;
		...
		*/		
		...
		Data_structure
			...
			&tt=*t.get_node();
		In_face_iterator
			bottom_left_it(follow_curve(tt1,left_cv,CGAL_SMALLER)),
			mid_left_it(follow_curve(tt1,left_cv,CGAL_EQUAL)),
			top_left_it(follow_curve(tt1,left_cv,CGAL_LARGER)),
			bottom_right_it(follow_curve(tt,right_cv,CGAL_SMALLER)),
			mid_right_it(follow_curve(tt,right_cv,CGAL_EQUAL)),
			top_right_it(follow_curve(tt,right_cv,CGAL_LARGER));
	...
	*/
	Self& preprocess()
	{
		...
		if (not_empty)
		{
			...
			typename list<pointer>::iterator it=representatives.begin();
	...
	current CGAL_Trapezoidal_map_2<Traits,X_curve_plus> needs update
	Postconditions:
	The output is true iff the depth of the Trapezoidal Tree is more then
	DepthThreshold times log of the curve count or the Trapezoidal Tree's size is
	more then SizeThreshold times the log of the last count.
	*/
...
*/
		return
			depth()>(get_depth_threshold()*log(sz+1))
...
*/
	...
	bool
	Description:
	uses needs_update to determine whether the CGAL_Trapezoidal_map_2<Traits,X_curve_plus> needs update
	and calls preprocess accordingly
	Postcondition:
	the return value is true iff preprocessing took place.
	*/
	bool update()
	{
		if (needs_update()) {preprocess();return true;}
		return false;
	}
	...
	/* ---- output ---------------*/
	friend CGAL_STD_IO_ ostream& operator<<(CGAL_STD_IO_ ostream &cout,const typename CGAL_Trapezoidal_map_2<Traits,X_curve_plus>::In_face_iterator& it)
	{
		cout << "In_face_iterator(";
		if (it.operator->()) cout << *it; else cout <<"end";
		return cout << ")"
...
*/
	...
	void locate_opt_push(pointer cv)
	{
		prev_cv=last_cv;
		last_cv=cv;
	}
	void locate_opt_empty()
	{
		last_cv=prev_cv=0;
	}
	bool locate_opt_swap(pointer& cv)
	{
		cv=last_cv;
		last_cv=prev_cv;
		prev_cv=cv;
	...
	void locate_optimization(const Point& p,pointer& tr,Locate_type& lt)
	{
		// optimization
		if(
...
template < class Traits,class X_curve_plus>
CGAL_STD_IO_ ostream& operator<<(CGAL_STD_IO_ ostream &cout,const CGAL_Trapezoidal_map_2<Traits,X_curve_plus>& tm)
{
	return cout << tm.get_trapezoid_tree()
...
template < class Traits,class X_curve_plus>
CGAL_STD_IO_ ostream& operator<<(CGAL_STD_IO_ ostream &cout,const __X_trapezoid<Traits, X_curve_plus>& t)
{
	typedef CGAL_Trapezoidal_map_2<Traits,X_curve_plus> TM;
	typedef __X_trapezoid<Traits, X_curve_plus> X_trapezoid;
	typedef X_trapezoid* pointer;
	cout << "(";
	if (!t.is_left_unbounded()) cout << t.left(); else cout << "-oo";
	cout << ",";
	if (!t.is_right_unbounded()) cout << t.right(); else cout << "+oo";
	cout << ","
		...
		;
	if (!t.is_bottom_unbounded()) cout << t.bottom(); else cout << "-oo";
	cout << ",";
	if (!t.is_top_unbounded()) cout << t.top(); else cout << "+oo";
	cout << ",neighbours("
	...
	{
		0,
			(X_trapezoid*)CGAL_TRAPEZOIDAL_MAP_2_DELETE_SIGNATURE,
	...
	for (j=null_size;j<=max_size;j++)
	{
		for (i=0;i<size;i++)
			...
			{
				strcat(name[i],"=");
				strcat(name[i],name[j]);
				break;
			}
			if (i==size)
			{
				value[size]=value[j];
				strcpy(name[size++],name[j]);
			}
	}
	if (size==null_size) {cout << "none";}
...
__X_trapezoid<Traits,X_curve_plus> */
template < class Traits,class X_curve_plus>
class __X_trapezoid : public CGAL_Handle
{
public:
	typedef typename Traits::Point Point;
	typedef typename Traits::X_curve X_curve;
	...
	typedef __X_trapezoid<Traits, X_curve_plus> X_trapezoid;
	typedef X_trapezoid* pointer;
	typedef CGAL_Pm_tentuple<Traits*,Point,Point,curve,curve,unsigned char,pointer,pointer,pointer,pointer> Boundary_type;
	typedef CGAL_Trapezoidal_map_2<Traits,X_curve_plus> TM;
	typedef TM::Unbounded Unbounded;
	friend class CGAL_Trapezoidal_map_2<Traits,X_curve_plus>;
	friend class CGAL_Trapezoidal_map_2<Traits,X_curve_plus>::Around_point_circulator;
	friend class CGAL_Trapezoidal_map_2<Traits,X_curve_plus>::In_face_iterator;
	typedef CGAL_DAG< X_trapezoid,TM::Active_right_degenerate_curve_trapezoid > Data_structure;
	
	// implementation for trapezoids with two sides parallel to y axis
	/* X_trapezoid abbriviates Planar trapezoid with vertical parallel left and right sides.
...
#ifndef CGAL_TRAPEZOIDAL_MAP_2_DEBUG
#ifndef CGAL_TEMPLATE_FRIEND_CLASS_FOR_NESTED_CLASS_BUG
...
public:
	...
	void set_left(const Point& p) {ptr()->e1=p;ptr()->e5&=~CGAL_TRAPEZOIDAL_MAP_2_LEFT_UNBOUNDED;}
	void set_right(const Point& p) {ptr()->e2=p;ptr()->e5&=~CGAL_TRAPEZOIDAL_MAP_2_RIGHT_UNBOUNDED;}
	void set_bottom(const curve& cv) {ptr()->e3=cv;ptr()->e5&=~CGAL_TRAPEZOIDAL_MAP_2_BOTTOM_UNBOUNDED;}
	void set_top(const curve& cv) {ptr()->e4=cv;ptr()->e5&=~CGAL_TRAPEZOIDAL_MAP_2_TOP_UNBOUNDED;}
	void set_left_unbounded() {ptr()->e5|=CGAL_TRAPEZOIDAL_MAP_2_LEFT_UNBOUNDED;}
	void set_right_unbounded() {ptr()->e5|=CGAL_TRAPEZOIDAL_MAP_2_RIGHT_UNBOUNDED;}
	void set_bottom_unbounded() {ptr()->e5|=CGAL_TRAPEZOIDAL_MAP_2_BOTTOM_UNBOUNDED;}
	void set_top_unbounded() {ptr()->e5|=CGAL_TRAPEZOIDAL_MAP_2_TOP_UNBOUNDED;}
...
public:
	__X_trapezoid(Traits* traits){PTR=new Boundary_type(traits,TM::POINT_AT_LEFT_TOP_INFINITY,TM::POINT_AT_RIGHT_BOTTOM_INFINITY,TM::infinite_dummy_curve,TM::infinite_dummy_curve,CGAL_TRAPEZOIDAL_MAP_2_TOTALLY_UNBOUNDED,0,0,0,0);node=0;}
	__X_trapezoid(Traits* traits,const Point& l ,const Point&r,const curve& b,const curve &t,unsigned char c=CGAL_TRAPEZOIDAL_MAP_2_BOUNDED,
	...
	__X_trapezoid(Traits* traits,const Point* l ,const Point* r , const curve* b ,const curve *t ,
		X_trapezoid *lb=0,X_trapezoid *lt=0,X_trapezoid *rb=0,X_trapezoid *rt=0,
		Data_structure* p=0){PTR=new Boundary_type(traits,l ? *l : TM::POINT_AT_LEFT_TOP_INFINITY,r ? *r : TM::POINT_AT_RIGHT_BOTTOM_INFINITY,b ? *b : TM::infinite_dummy_curve,t ? *t : TM::infinite_dummy_curve,
		(l ? 0 : CGAL_TRAPEZOIDAL_MAP_2_LEFT_UNBOUNDED) | 
		(r ? 0 : CGAL_TRAPEZOIDAL_MAP_2_RIGHT_UNBOUNDED) | 
		(b ? 0 : CGAL_TRAPEZOIDAL_MAP_2_BOTTOM_UNBOUNDED) | 
		(t ? 0 : CGAL_TRAPEZOIDAL_MAP_2_TOP_UNBOUNDED),
		lb,lt,rb,rt);node=p;}
	__X_trapezoid(const X_trapezoid& tr) : CGAL_Handle(tr){node=tr.node;}
	~__X_trapezoid() {}
	/*
	...
	*/
	...
	const Point& left() const {return !is_left_unbounded() ? ptr()->e1 : TM::POINT_AT_LEFT_TOP_INFINITY;}
	const Point& right() const {return !is_right_unbounded() ? ptr()->e2 : TM::POINT_AT_RIGHT_BOTTOM_INFINITY;}
	// filters out the infinite case where at returns predefined dummy values
	const curve& bottom() const {return !is_bottom_unbounded() ?  ptr()->e3 : TM::infinite_dummy_curve;}
	const curve& top() const {return !is_top_unbounded() ?	ptr()->e4 : TM::infinite_dummy_curve;}
	unsigned char boundedness() const {return ptr()->e5;}
	bool is_left_unbounded() const {return (ptr()->e5&CGAL_TRAPEZOIDAL_MAP_2_LEFT_UNBOUNDED)!=0;}
	bool is_right_unbounded() const {return (ptr()->e5&CGAL_TRAPEZOIDAL_MAP_2_RIGHT_UNBOUNDED)!=0;}
	bool is_bottom_unbounded() const {return (ptr()->e5&CGAL_TRAPEZOIDAL_MAP_2_BOTTOM_UNBOUNDED)!=0;}
	bool is_top_unbounded() const {return (ptr()->e5&CGAL_TRAPEZOIDAL_MAP_2_TOP_UNBOUNDED)!=0;}
	bool is_unbounded() const
	{
		...
		return (ptr()->e5&CGAL_TRAPEZOIDAL_MAP_2_TOTALLY_UNBOUNDED)!=0;
	}
	...
	bool is_active() const {return right_bottom_neighbour()!=(pointer)CGAL_TRAPEZOIDAL_MAP_2_DELETE_SIGNATURE;}
	...
	void remove(Data_structure* left=0)
	{
		...
		set_rb((pointer)CGAL_TRAPEZOIDAL_MAP_2_DELETE_SIGNATURE);
	...
	bool is_valid() const
	{
		...
		if (is_active())
		{
			if (get_node() && **get_node()!=*this)
			{
				...
				cerr << "\nget_node= " << (*get_node())->left() << "," << (*get_node())->right() << "," << (*get_node())->bottom() << "," << (*get_node())->top() << flush;
			...
			if (!is_degenerate())
			{
				...
				{
					cerr << "\nleft=" << left() << " right=" << right() << " bottom=" << bottom() << " top=" << top() << flush;
			...
			points should be on the top() and bottom() curves.
				In any case none of the geometric boundaries should be unbounded */
				if (
					is_bottom_unbounded()||
					is_top_unbounded()||
					is_left_unbounded()||
...
#include <CGAL/Trapezoidal_map_2.C>
#endif

#endif// CGAL_TRAPEZOIDAL_MAP_2_H
