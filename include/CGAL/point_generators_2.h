...
template < class P, class Creator = CGAL_Creator_uniform_2<double,P> >
#else
template < class P, class Creator >
#endif
class CGAL_Random_points_in_disc_2 : public CGAL__Random_generator_base<P>{
    void generate_point();
public:
    typedef CGAL_Random_points_in_disc_2<P,Creator> This;
    CGAL_Random_points_in_disc_2( double r = 1,
...
template < class P, class Creator >
void
CGAL_Random_points_in_disc_2<P,Creator>::
generate_point() {
    double alpha = _rnd.get_double() * 2.0 * M_PI;
    double r     = d_range * sqrt( _rnd.get_double());
    Creator creator;
    d_item = creator( r * cos(alpha), r * sin(alpha));
}
...
template < class P, class Creator >
#endif
class CGAL_Random_points_on_circle_2
    : public CGAL__Random_generator_base<P> {
    void generate_point();
public:
    typedef CGAL_Random_points_on_circle_2<P,Creator> This;
    CGAL_Random_points_on_circle_2( double r = 1,
                                    CGAL_Random& rnd = CGAL_random)
        // g is an input iterator creating points of type `P' uniformly
...
template < class P, class Creator >
void
CGAL_Random_points_on_circle_2<P,Creator>::
generate_point() {
    double alpha = _rnd.get_double() * 2.0 * M_PI;
    Creator creator;
    d_item = creator( d_range * cos(alpha), d_range * sin(alpha));
}
...
template < class P, class Creator >
#endif
class CGAL_Random_points_in_square_2
    : public CGAL__Random_generator_base<P> {
    void generate_point();
public:
    typedef CGAL_Random_points_in_square_2<P,Creator> This;
    CGAL_Random_points_in_square_2( double a = 1,
                                    CGAL_Random& rnd = CGAL_random)
        // g is an input iterator creating points of type `P' uniformly
...
template < class P, class Creator >
void
CGAL_Random_points_in_square_2<P,Creator>::
generate_point() {
    Creator creator;
...
template < class P, class Creator >
#endif
class CGAL_Random_points_on_square_2
    : public CGAL__Random_generator_base<P> {
    void generate_point();
public:
    typedef CGAL_Random_points_on_square_2<P,Creator> This;
    CGAL_Random_points_on_square_2( double a = 1,
                                    CGAL_Random& rnd = CGAL_random)
        // g is an input iterator creating points of type `P' uniformly
...
template < class P, class Creator >
void
CGAL_Random_points_on_square_2<P,Creator>::
generate_point() {
...
template < class P, class Creator >
#endif
class CGAL_Random_points_on_segment_2
: public CGAL__Random_generator_base<P> {
    P _p;
    P _q;
    void generate_point();
public:
    typedef CGAL_Random_points_on_segment_2<P,Creator> This;
    CGAL_Random_points_on_segment_2( const P& p = P( -1, 0),
                                     const P& q = P(  1, 0),
                                     CGAL_Random& rnd = CGAL_random)
        // g is an input iterator creating points of type `P' uniformly
    ...
    : CGAL__Random_generator_base<P>( max( max( CGAL_to_double(p.x()),
                                                CGAL_to_double(q.x())),
                                           max( CGAL_to_double(p.y()),
                                                CGAL_to_double(q.y()))),
                                      rnd) , _p(p), _q(q) {
        generate_point();
    }
...
template < class P, class Creator >
void
CGAL_Random_points_on_segment_2<P,Creator>::
generate_point() {
    double la = _rnd.get_double();
    double mu = 1.0 - la;
    Creator creator;
    d_item = creator( mu * CGAL_to_double(_p.x()) +
...
template < class P >
class CGAL_Points_on_segment_2
: public CGAL__Generator_base<P> {
    P _p;
    P _q;
    size_t  d_i;
    size_t  d_mx;
    void generate_point();
public:
    typedef CGAL_Points_on_segment_2<P> This;
    CGAL_Points_on_segment_2() {}
    CGAL_Points_on_segment_2( const P& p, const P& q,
                              size_t mx, size_t i = 0)
    : CGAL__Generator_base<P>( max( max( CGAL_to_double(p.x()),
                                         CGAL_to_double(q.x())),
                                    max( CGAL_to_double(p.y()),
                                         CGAL_to_double(q.y())))),
...
template < class P >
void
CGAL_Points_on_segment_2<P>::
generate_point() {
    d_item = _p + (_q-_p) * d_i / (d_mx-1);
}
...
OutputIterator
CGAL__points_on_square_grid_2( double a, size_t n, OutputIterator o,
                               const P*)
{
    return CGAL_points_on_square_grid_2( a, n, o,
...
CGAL_points_on_square_grid_2( double a, size_t n, OutputIterator o)
{
    return CGAL__points_on_square_grid_2( a, n, o, value_type(o));
}
...
void CGAL__perturb_points_2( ForwardIterator first,
                            ForwardIterator last,
                            double xeps,
                            double yeps,
                            CGAL_Random& rnd,
                            const P*)
{
    CGAL_perturb_points_2(first,last,xeps,yeps,rnd,
                          CGAL_Creator_uniform_2<double,P>());
}
...
void CGAL_perturb_points_2( ForwardIterator first,
                            ...
                            CGAL_Random& rnd)
{
    CGAL__perturb_points_2(first,last,xeps,yeps,rnd,value_type(first));
}
                       ...
                       Creator creator)
{
    ...
    for ( size_t i = 0; i < n; i++) {
        ...
        double mu = 1.0 - la;
        *first2++ = creator(mu * CGAL_to_double(p.x()) +
...
OutputIterator CGAL__random_collinear_points_2(
                       ...
                       P*)
{
    return CGAL_random_collinear_points_2( first, last, n, first2, rnd,
                                       CGAL_Creator_uniform_2<double,P>());
}
...
OutputIterator CGAL_random_collinear_points_2(
    ...
    // Precondition: The expression `CGAL_to_double((*first).x()
