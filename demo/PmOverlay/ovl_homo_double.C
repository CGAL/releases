...
int main()
{
  ...
  cout << "Creating the first map.\n";
  
  for (i=0; i<nr_curves; i++){
    f_stream_in1 >> i1 >> i2 >> c;
    cout << " - inserting " << Curve(A[i1-1], A[i2-1]) << endl;
  ...
  cout << "The edges of pm1:\n";
  Planar_map::Halfedge_iterator h_ci;
  for (h_ci=pm1.halfedges_begin(); h_ci!=pm1.halfedges_end(); ++h_ci){
    cout << (*h_ci).curve() << endl;
  }
